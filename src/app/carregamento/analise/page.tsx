...
export default function Teste() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      alert('Erro ao carregar dados do dashboard. Verifique a conexão com a API.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={currentPage === Math.ceil(docasFiltradas.length / itemsPerPage) - 1}
          ...
          <div className="flex gap-4 mb-6">
            ...
            >
              ...
              <span className="text-sm">
                {todasDocas.filter((d) => d.status === "ocupada").length} docas ocupadas
              </span>
            </button>
            ...
            >
              ...
              <span className="text-sm">
                {todasDocas.filter((d) => d.status === "liberada").length} rotas liberadas
              </span>
            </button>
          </div>
          ...
          <div className="relative">
            {docasFiltradas.length === 0 ? (
              <div className={`p-8 rounded-lg border ${getBorderColor()} ${getBgColor()} text-center`}>
                ...
                <h4 className={`font-bold text-lg mb-1 ${getTextColor()}`}>
                  {viewMode === "em_uso" ? "Nenhuma doca em uso" : "Nenhuma rota liberada"}
                </h4>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-4">
                    {colunaEsquerda.map((doca) => (
                      <div key={doca._id || doca.id} className={`p-4 border rounded-lg transition-all duration-300 ${getBorderColor()} ${getBgColor()}`}>
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg ${getBadgeColor()}`}>
                              {viewMode === "em_uso" ? <Truck className="w-6 h-6" /> : <CheckCircle className="w-6 h-6" />}
                            </div>
                            <div>
                              <div className="font-bold text-xl">Doca {doca.id}</div>
                              <div className={`text-sm font-medium ${getTextColor()}`}>
                                {viewMode === "em_uso" ? "Em Carregamento" : "Rota Liberada"}
                              </div>
                            </div>
                          </div>
                          {doca.tipoVeiculo && (
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="font-medium">{doca.motorista || "Não informado"}</div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="font-medium">{doca.cidadeDestino || "Não informado"}</div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <div className="text-xs text-gray-500">Placa do Veículo</div>
                              <div className="font-medium font-mono">{doca.placaVeiculo || "Não informada"}</div>
                            </div>
                          </div>

                          {doca.carga && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="text-xs text-gray-500 mb-2">Carga</div>
                          ...
                          {viewMode === "liberadas" && doca.horarioSaida && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="text-xs text-gray-500">Horário de Saída</div>
                              <div className="font-medium">{doca.horarioSaida}</div>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                {Math.ceil(docasFiltradas.length / itemsPerPage) > 1 && (
                  <div className="flex justify-center gap-2 mt-6">
                    {Array.from({ length: Math.ceil(docasFiltradas.length / itemsPerPage) }).map((_, index) => (
                      <button
                        key={index}
