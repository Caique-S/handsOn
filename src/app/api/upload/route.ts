...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const db = client.db('sistema-carregamento'); // ← Adicione o nome do seu banco aqui
    ...
    console.log('7. Inserindo na coleção "uploads"...');

    // Verificar se a coleção existe, se não, criar
    const collections = await db.listCollections({ name: 'uploads' }).toArray();
    if (collections.length === 0) {
      console.log('✓ Criando coleção "uploads"...');
      await db.createCollection('uploads');
    }

    // Inserir documento
    const result = await db.collection('uploads').insertOne(uploadDocument);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const client = await clientPromise;
    const db = client.db('sistema-carregamento');
    ...
    const total = await db.collection('uploads').countDocuments();
